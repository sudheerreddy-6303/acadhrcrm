...
// Fetch tutors from the AcadHr schema (same server) and bring them into the
// CRM Tutors list as UNREGISTERED records, reusing the existing import flow.
export default function FetchTutors() {
  const navigate = useNavigate();
  const [status, setStatus] = useState(null);   // { reachable, tutorCount, message }
  const [preview, setPreview] = useState(null);  // { columns, detectedFields, sample }
  const [analysis, setAnalysis] = useState(null);// dedup result
  const [imported, setImported] = useState(null);// { count }
  ...
  const runPreview = async () => {
    ...
    try {
      setPreview(await api.get('/acadhr-sync/tutors/preview?limit=5'));
    } catch (e) { setError(e.message); } finally { setBusy(''); }
  };

  // Fetch all rows, then dedup against the CRM (same checks the Import page uses).
  const runFetch = async () => {
    setBusy('fetch'); setError(''); setImported(null);
    try {
      const f = await api.post('/acadhr-sync/tutors/fetch', {});
  ...
  const runImport = async () => {
    ...
    try {
      const res = await api.post('/leads/import', { type: 'tutors', rows: analysis.toImport, registration: 'registered' });
  ...
  return (
    <div>
      <div className="page-head">
        <div>
          <h2>Fetch Tutors from AcadHr</h2>
          <p className="muted">Pull tutors from the AcadHr database into the CRM Tutors list under Registration Followup.</p>
        </div>
      </div>
      ...
      {status && status.reachable === false && (
        <div className="alert">
          {status.message || 'Could not reach the AcadHr tutors table.'}
        </div>
      )}
      ...
      {status && status.reachable && (
        <>
          <p className="muted">
            Found <strong>{status.tutorCount}</strong> tutor(s) in <code>{status.sourceDb}.tutors</code>.
            Imported tutors appear in the <strong>Tutors</strong> list under <strong>Registration Followup</strong> (registered).
          </p>
            ...
            <div className="table-wrap" style={{ marginTop: 8 }}>
              <p className="muted" style={{ margin: '0 0 8px' }}>
                Detected fields: {preview.detectedFields.length
                  ...
                  : <em>none — check the column names in acadhr.tutors</em>}
              </p>
          ...
          {analysis && !imported && (
            <>
              ...
              <button className="btn-primary" onClick={runImport} disabled={busy === 'import' || analysis.report.willImport === 0}>
                {busy === 'import' ? 'Importing…' : `Import ${analysis.report.willImport} into Tutors`}
              </button>
            </>
          )}
          ...
          {imported && (
            <div className="import-ok" style={{ marginTop: 14 }}>
              Imported <strong>{imported.count}</strong> tutor(s) into the CRM under Registration Followup.
              <div style={{ marginTop: 10 }}>
                <button className="btn-primary" onClick={() => navigate('/tutors?registration=registered')}>
                  View in Tutors
                </button>
              </div>
            </div>
